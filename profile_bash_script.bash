#!/usr/bin/env bash
#
# Submit this single file to SLURM:
#   sbatch profile_self_attention.slurm
#
# Outputs (in $SLURM_SUBMIT_DIR/out-$SLURM_JOB_ID/):
#   - results.csv
#   - plot_time_ms.png
#   - plot_memory_bytes.png
#   - plot_flops_count.png
#   - plot_achieved_gflops_per_s.png
#   - report.txt
#
# You can pass a GitHub link to record in the report:
#   sbatch --export=ALL,GITHUB_LINK="https://github.com/<you>/<repo>" profile_self_attention.slurm
#
#SBATCH -J attn-prof
#SBATCH -p gpu
#SBATCH -N 1
...
OUTDIR="${SLURM_SUBMIT_DIR:-$PWD}/out-${SLURM_JOB_ID:-local}"
mkdir -p "$OUTDIR"
...
# Use the python that has PyTorch + Matplotlib installed
PYTHON_BIN="${PYTHON_BIN:-python3}"

# Write the Python driver to a temp file (kept next to outputs for reproducibility)
PYFILE="$OUTDIR/run_attn_profile.py"
cat > "$PYFILE" <<'PYCODE'
...
def make_config(outdir: str, github_link: str) -> Config:
    return Config(
        outdir=outdir,
    ...
    for L in cfg.lengths:
        ...
        for t in range(trials):
            ...
            if device == "cuda":
                ...
                dt_ms, _ = cuda_time_ms(lambda: model(x))
...
def write_report(all_results: List[Dict], cfg: Config, outdir: str):
    ...
    if cfg.github_link:
        lines.append(f"- Code on GitHub: {cfg.github_link}")
    lines.append("")
...
def main():
    ...
    parser.add_argument("--github_link", type=str, default=os.environ.get("GITHUB_LINK",""))
    args = parser.parse_args()

    cfg = make_config(args.outdir, args.github_link)
    ...
    if cfg.github_link:
        print(f"GitHub link recorded: {cfg.github_link}")
...
# Run
echo "[INFO] Launching Python profiler script..."
"$PYTHON_BIN" "$PYFILE" --outdir "$OUTDIR"

echo "[INFO] All artifacts in: $OUTDIR"